...
### 3.1 Двухуровневый стандарт контрактов (Two-Tier Contracts Standard)

В фабрике применяется обязательное разделение контрактов на два уровня:
1. **Тип 1: Универсальный Протокольный Контракт (`contracts/protocol/` или `contracts/front/`):**
   - Описывает шину/мета-протокол общения, транспортные конверты (RPC/REST Envelope), манифест возможностей (Capabilities/Discovery), формат сессионного контекста и структуру типизированных ошибок с указанием `failed_parameter` для точечного отката FSM-оркестратором.
   - Не зависит от конкретной бизнес-области (CRM, ERP, WMS).
2. **Тип 2: Доменный Контракт Компонента (`contracts/domain/` или `contracts/api/`):**
   - Описывает конкретные бизнес-функции (входные и выходные структуры данных, бизнес-валидацию).
   - **Обязательное правило парной разработки:** Доменный контракт всегда разрабатывается **строго совместно со встроенным мок-пакетом (`mocks/`)** (Zero-Manual Mocks), воспроизводящим эталонные позитивные ответы и триггеры ошибок.

### 3.2 Матрица выбора формата контракта
...
### 3.3 Структура каталога контрактов и моков
```text
contracts/
├── front/       # Спецификации UI/клиентского интерфейса (Front Chat API и др.)
├── protocol/    # Универсальные протоколы вызова и обнаружения сервисов
├── domain/      # Доменные схемы бизнес-функций и сущностей
└── events/      # AsyncAPI схемы очередей и событий
